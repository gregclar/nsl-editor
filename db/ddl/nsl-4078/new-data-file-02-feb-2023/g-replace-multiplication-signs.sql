...
select count(*)
  from loader_name
 where loader_batch_id = ( select id from loader_batch where lower(name) = lower('APC List 103 draft 02 Feb 2023'))
   and simple_name ~ ' ×[A-z]'
;
...
update loader_name
 ...
 where loader_batch_id =  (
    select id
      from loader_batch
 where lower(name) = lower('APC List 103 draft 02 Feb 2023'))
   and simple_name ~ ' ×[A-z]';
