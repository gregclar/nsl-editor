...
as
 SELECT subq.display_as,
   ...
   FROM ( SELECT 'Loader Batch in stack'::text AS display_as,
            ...
            (loader_batch.name || ' A batch '::text) || loader_batch.name::text AS order_by
           FROM loader_batch
        UNION
         SELECT 'Batch Review in stack'::text AS display_as,
            ...
            br.created_at,
            br.created_at,
            lb.name || (('A batch '::text || lb.name::text) || ' B review '::text) || br.name::text AS order_by
        ...
        UNION
         SELECT 'Review Period in stack'::text AS display_as,
            ...
            brp.created_at,
            brp.start_date,
            lb.name || (((('A batch '::text || lb.name::text) || ' B review '::text) || br.name::text) || ' C period '::text) || brp.start_date AS order_by
        ...
        UNION
         SELECT 'Batch Reviewer in stack'::text AS display_as,
            ...
            brer.created_at,
            brer.created_at,
            lb.name || (((('A batch '::text || lb.name::text) || ' B review '::text) || br.name::text)  || ' D reviewer '::text) || users.name::text AS order_by
