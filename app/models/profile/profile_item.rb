...
#  draft(A boolean to indicate this profile item is in draft mode and is not publicly available.)                                                                                                                                                       :boolean          default(TRUE), not null
#  lock_version(A system field to manage row level locking.)                                                                                                                                                                                            :integer          default(0), not null
#  profile_general_ref(A placeholder to indicate this profile item is for a list of references available in profile_references. 1=is a profile_reference data, null = not a profile reference. Used to constrain an item type to only one object type.) :boolean          default(FALSE), not null
...
# Indexes
#
#  profile_item_instance_i  (instance_id)
#  profile_item_telement_i  (tree_element_id)
#  profile_item_text_i      (profile_text_id)
#
# Foreign Keys
#
