...
#
class Search::OnTreeVersionElement::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTreeVersionElement::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeVersionElement::Predicate - #{s}")
  end

  def inspect
    "Search::OnTreeVersionElement::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnTreeVersionElement::FieldRule::RULES.key?(field)
      field
    elsif Search::OnTreeVersionElement::FieldRule::RULES.key?(
      Search::OnTreeVersionElement::FieldAbbrev::ABBREVS[field]
    )
      Search::OnTreeVersionElement::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search tree version element for: #{field}. You may need to try another
