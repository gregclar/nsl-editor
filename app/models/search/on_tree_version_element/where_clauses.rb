...
#
# Build where clauses for TreeVersionElement searches.
class Search::OnTreeVersionElement::WhereClauses
  attr_reader :sql

  DEFAULT_FIELD = "name_path:"
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeVersionElement::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnTreeVersionElement::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
