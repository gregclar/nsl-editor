...
#
# Search on Tree
class Search::OnTaxonomyReviewPeriod::Base
  attr_reader :results,
              :limited,
              :info_for_display,
              :rejected_pairings,
  ...
  def run_count_query(parsed_request)
    count_query = Search::OnTaxonomyReviewPeriod::CountQuery.new(parsed_request)
  ...
  def run_list_query(parsed_request)
    list_query = Search::OnTaxonomyReviewPeriod::ListQuery.new(parsed_request)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTaxonomyReviewPeriod::Base: #{s}")
  end
