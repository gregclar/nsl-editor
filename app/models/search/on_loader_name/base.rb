...
#
# Search on Loader Name
class Search::OnLoaderName::Base
  attr_reader :results,
              :limited,
              :info_for_display,
              :rejected_pairings,
  ...
  def run_count_query(parsed_request)
    count_query = Search::OnLoaderName::CountQuery.new(parsed_request)
  ...
  def run_list_query(parsed_request)
    list_query = Search::OnLoaderName::ListQuery.new(parsed_request)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnLoaderName::Base: #{s}")
  end
