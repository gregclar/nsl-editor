...
class Search::OnLoaderName::WhereClauses
  attr_reader :sql

  DEFAULT_FIELD = "scientific_name:"
  ...
  def debug(s)
    Rails.logger.debug("Search::OnLoaderName::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnLoaderName::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
