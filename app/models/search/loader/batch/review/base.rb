...
class Search::Loader::Batch::Review::Base
  attr_reader :results,
              :limited,
              :info_for_display,
              :rejected_pairings,
  ...
  def run_count_query(parsed_request)
    count_query = Search::Loader::Batch::Review::CountQuery.new(parsed_request)
  ...
  def run_list_query(parsed_request)
    list_query = Search::Loader::Batch::Review::ListQuery.new(parsed_request)
  ...
  def debug(s)
    Rails.logger.debug("Search::Loader::Batch::Review::Base: #{s}")
  end
