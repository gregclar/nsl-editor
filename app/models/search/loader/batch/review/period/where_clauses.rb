...
class Search::Loader::Batch::Review::Period::WhereClauses
  attr_reader :sql
  ...
  def debug(s)
    Rails.logger.debug("Search::Loader::Batch::Review::Period::WhereClause - #{s}")
  end
  ...
  def add_clause(field, value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::Loader::Batch::Review::Period::Predicate.new(
               field_or_default,
               value)
      apply_rule(rule)
      apply_order(rule)
