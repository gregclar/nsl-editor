...
#
class Search::OnLoaderBatch::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnLoaderBatch::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnLoaderBatch::Predicate - #{s}")
  end

  def inspect
    "Search::OnLoaderBatch::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnLoaderBatch::FieldRule::RULES.key?(field)
      field
    elsif Search::OnLoaderBatch::FieldRule::RULES.key?(
      Search::OnLoaderBatch::FieldAbbrev::ABBREVS[field]
    )
      Search::OnLoaderBatch::FieldAbbrev::ABBREVS[field]
    else
