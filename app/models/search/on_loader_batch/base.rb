...
class Search::OnLoaderBatch::Base
  attr_reader :results,
              :limited,
              :info_for_display,
              :rejected_pairings,
  ...
  def run_count_query(parsed_request)
    count_query = Search::OnLoaderBatch::CountQuery.new(parsed_request)
  ...
  def run_list_query(parsed_request)
    list_query = Search::OnLoaderBatch::ListQuery.new(parsed_request)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnLoaderBatch::Base: #{s}")
  end
