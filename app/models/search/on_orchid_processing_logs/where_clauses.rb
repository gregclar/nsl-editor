...
class Search::OnOrchidProcessingLogs::WhereClauses
  attr_reader :sql

  DEFAULT_FIELD = "latest:"
  ...
  def debug(s)
    Rails.logger.debug("Search::OnOrchidProcessingLogs::WhereClause - #{s}")
  end
  ...
  def add_clause(field = "taxon", value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnOrchidProcessingLogs::Predicate.new(field_or_default,
                                                           value)
      apply_rule(rule)
      apply_order(rule)
    end
