...
#
class Search::OnOrchidProcessingLogs::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnOrchidProcessingLogs::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
    @canon_value = build_canon_value(value)
    apply_scope
    @order = rule[:order] || " logged_at desc "
    process_value
  end

  def debug(s)
    Rails.logger.debug("Search::OnOrchidProcessingLogs::Predicate - #{s}")
  end

  def inspect
    "Search::OnOrchidProcessingLogs::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnOrchidProcessingLogs::FieldRule::RULES.key?(field)
      field
    elsif Search::OnOrchidProcessingLogs::FieldRule::RULES.key?(
      Search::OnOrchidProcessingLogs::FieldAbbrev::ABBREVS[field]
    )
      Search::OnOrchidProcessingLogs::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search orchids for: #{field}. You may need to try another
