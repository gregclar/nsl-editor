...
#
# Build where clauses for TreeElement searches.
class Search::OnTreeElement::WhereClauses
  attr_reader :sql

  DEFAULT_FIELD = "simple_name:"
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeElement::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnTreeElement::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
