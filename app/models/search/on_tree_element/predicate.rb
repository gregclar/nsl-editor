...
#
class Search::OnTreeElement::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTreeElement::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeElement::Predicate - #{s}")
  end

  def inspect
    "Search::OnTreeElement::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnTreeElement::FieldRule::RULES.key?(field)
      field
    elsif Search::OnTreeElement::FieldRule::RULES.key?(
      Search::OnTreeElement::FieldAbbrev::ABBREVS[field]
    )
      Search::OnTreeElement::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search tree element for: #{field}. You may need to try another
