...
class Search::OnTaxonomyVersionReviewPeriod::WhereClauses
  attr_reader :sql
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTaxonomyVersionReviewPeriod::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnTaxonomyVersionReviewPeriod::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
