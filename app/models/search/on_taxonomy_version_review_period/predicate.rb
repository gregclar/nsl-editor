...
#
class Search::OnTaxonomyVersionReviewPeriod::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTaxonomyVersionReviewPeriod::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTree::Predicate - #{s}")
  end

  def inspect
    "Search::OnTree::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    debug("build_canon_field for field: #{field}")
    if Search::OnTaxonomyVersionReviewPeriod::FieldRule::RULES.key?(field)
      field
    elsif Search::OnTaxonomyVersionReviewPeriod::FieldRule::RULES.key?(
      Search::OnTaxonomyVersionReviewPeriod::FieldAbbrev::ABBREVS[field]
    )
      Search::OnTaxonomyVersionReviewPeriod::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search taxonomy review period for: #{field}. You may need to try another
