...
#
class Search::OnTreeVersion::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTreeVersion::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeVersion::Predicate - #{s}")
  end

  def inspect
    "Search::OnTreeVersion::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnTreeVersion::FieldRule::RULES.key?(field)
      field
    elsif Search::OnTreeVersion::FieldRule::RULES.key?(
      Search::OnTreeVersion::FieldAbbrev::ABBREVS[field]
    )
      Search::OnTreeVersion::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search tree version for: #{field}. You may need to try another
