...
#
# Build where clauses for TreeVersion searches.
class Search::OnTreeVersion::WhereClauses
  attr_reader :sql

  DEFAULT_FIELD = "draft_name:"
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTreeVersion::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnTreeVersion::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
