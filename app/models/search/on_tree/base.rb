...
class Search::OnTree::Base
  ...
  def run_count_query(parsed_request)
    count_query = Search::OnTree::CountQuery.new(parsed_request)
  ...
  def run_list_query(parsed_request)
    list_query = Search::OnTree::ListQuery.new(parsed_request)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTree::Base: #{s}")
  end
