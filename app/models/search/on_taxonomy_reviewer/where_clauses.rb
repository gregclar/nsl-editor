...
#
# Build where clauses for Tree searches.
class Search::OnTaxonomyReviewer::WhereClauses
  attr_reader :sql
  ...
  def debug(s)
    Rails.logger.debug("Search::OnTaxonomyReviewer::WhereClause - #{s}")
  end
  ...
  def add_clause(field = 'taxon', value)
    ...
    if field.blank? && value.blank?
      ...
      rule = Search::OnTaxonomyReviewer::Predicate.new(field_or_default,
                                             value)
      apply_rule(rule)
      apply_order(rule)
    end
  ...
  def apply_rule(rule)
    if rule.tokenize
      ...
      @sql = @sql.send(rule.scope_, rule.value).reorder("username")
    else
