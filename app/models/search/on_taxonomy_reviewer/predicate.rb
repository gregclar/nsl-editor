...
#
class Search::OnTaxonomyReviewer::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTaxonomyReviewer::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def build_canon_field(field)
    if Search::OnTaxonomyReviewer::FieldRule::RULES.key?(field)
      field
    elsif Search::TaxonomyReviewer::FieldRule::RULES.key?(
      Search::TaxonomyReviewer::FieldAbbrev::ABBREVS[field]
    )
      Search::TaxonomyReviewer::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search taxonomy review for: #{field}. You may need to try another
