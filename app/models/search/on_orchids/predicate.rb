...
#
class Search::OnOrchids::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnOrchids::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def debug(s)
    Rails.logger.debug("Search::OnOrchids::Predicate - #{s}")
  end

  def inspect
    "Search::OnOrchids::Predicate: canon_field: #{@canon_field}"
  end
  ...
  def build_canon_field(field)
    if Search::OnOrchids::FieldRule::RULES.key?(field)
      field
    elsif Search::OnOrchids::FieldRule::RULES.key?(
      Search::OnOrchids::FieldAbbrev::ABBREVS[field]
    )
      Search::OnOrchids::FieldAbbrev::ABBREVS[field]
    else
