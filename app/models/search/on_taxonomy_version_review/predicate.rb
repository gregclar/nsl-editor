...
#
class Search::OnTaxonomyVersionReview::Predicate
  ...
  def initialize(field, value)
    ...
    rule = Search::OnTaxonomyVersionReview::FieldRule::RULES[@canon_field] || EMPTY_RULE
    @is_null = value.blank?
    apply_rule(rule)
  ...
  def build_canon_field(field)
    if Search::OnTree::FieldRule::RULES.key?(field)
      field
    elsif Search::OnTree::FieldRule::RULES.key?(
      Search::OnTree::FieldAbbrev::ABBREVS[field]
    )
      Search::OnTree::FieldAbbrev::ABBREVS[field]
    else
      raise "Cannot search tree for: #{field}. You may need to try another
