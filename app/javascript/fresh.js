...
(function() {
  ...
  window.loadStandardDetails = function(event, inFocus, tabWasClicked = false) {
    ...
    info('window.loadStandardDetails starting');
    $('#search-result-details').show();
    $('#search-result-details').removeClass('hidden');
    ...
    info(`loadDetails url: ${url}`);
    $('#search-result-details').load(url, function() {
      recordCurrentActiveTab(record_type);
      if (tabWasClicked) {
        info('tab clicked loadDetails');
        if ($('.give-me-focus')) {
          return info('give-me-focus ing - changed so not .give-me-focus ing because clicked a tab resulted in focus switching to the first record');
        } else {
          $('.give-me-focus').focus()
          info('just focus the tab');
          return $('li.active a.tab').focus();
        }
      } else {
        return info('tab was not clicked');
      }
    });
    // $('li.active a.tab').focus()   ## new
    info('loadDetails after load url');
    return event.preventDefault();
  };
