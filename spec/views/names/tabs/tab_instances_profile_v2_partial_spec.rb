...
RSpec.describe "names/tabs/_tab_instances_profile_v2.html.erb", type: :view do
  ...
  let(:profile_product) { FactoryBot.create(:profile_product, name: user.profile_v2_context.product) }
  let(:reference) { FactoryBot.create(:reference) }
  let!(:instance_type) { FactoryBot.create(:instance_type, name: "secondary reference") }

  before do
    allow(user).to receive_message_chain(:profile_v2_context, :product).and_return(profile_product.name)
    allow(Profile::Product).to receive(:find_by).with(name: profile_product.name).and_return(profile_product)
    allow(profile_product).to receive(:reference).and_return(reference)
  ...
  context "for when the name is not a duplicate" do
    ...
    it "displays the form for creating a new instance" do
      ...
      expect(rendered).to include("The new instance will be attached to the #{profile_product.name} product reference:")
