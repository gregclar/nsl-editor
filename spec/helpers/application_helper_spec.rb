...
RSpec.describe(ApplicationHelper, type: :helper) do
  ...
  describe "#tab_display_text" do
    let(:product) { double("Product", name: "TestProduct") }
    let(:service) { double("ProductTabService") }

    before do
      allow(Products::ProductTabService).to receive(:call).with(product).and_return(service)
    end

    context "when show_product_name_for_model?(:instance) returns true" do
      before { allow(service).to receive(:show_product_name_for_model?).with(:instance).and_return(true) }

      it "returns product name and humanized tab_type when custom_text is nil" do
        expect(helper.tab_display_text(product, "details")).to eq("TestProduct Details")
      end

      it "returns product name and custom_text when custom_text is provided" do
        expect(helper.tab_display_text(product, "details", "Custom Tab")).to eq("TestProduct Custom Tab")
      end
    end

    context "when show_product_name_for_model?(:instance) returns false" do
      before { allow(service).to receive(:show_product_name_for_model?).with(:instance).and_return(false) }

      it "returns only humanized tab_type when custom_text is nil" do
        expect(helper.tab_display_text(product, "details")).to eq("Details")
      end

      it "returns only custom_text when custom_text is provided" do
        expect(helper.tab_display_text(product, "details", "Custom Tab")).to eq("Custom Tab")
      end
    end
  end

  describe "#tab_link_id" do
    let(:product) { double("Product", name: "TestProduct") }
    let(:service) { double("ProductTabService") }
    ...
    context "when show_product_name_for_model?(:instance) returns true" do
      ...
      it "returns id with product name and default suffix when custom_suffix is nil" do
        expect(helper.tab_link_id(product, "base")).to eq("base-testproduct-tab")
      end

      it "returns id with product name and custom suffix when custom_suffix is provided" do
        expect(helper.tab_link_id(product, "base", "custom")).to eq("base-testproduct-custom")
      end
    end
    ...
    context "when show_product_name_for_model?(:instance) returns false" do
      ...
      it "returns id with default suffix when custom_suffix is nil" do
        expect(helper.tab_link_id(product, "base")).to eq("base-tab")
      end

      it "returns id with custom suffix when custom_suffix is provided" do
        expect(helper.tab_link_id(product, "base", "custom")).to eq("base-custom")
      end
    end
  end
end
